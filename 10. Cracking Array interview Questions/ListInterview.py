...
def findPairs(arr, k):
    ...
    for value in arr:
        if value-k not in complement_dc:
            complement_dc[value] = [k-value, 0]
    for value in arr:
        if k-value in complement_dc and [k-value, 0] in complement_dc.values() and value != k-value:
            print("({}, {})".format(value,k-value))
            counter += 1
            if k-value in complement_dc:
                complement_dc[k-value][1] = 1
